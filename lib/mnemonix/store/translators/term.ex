defmodule Mnemonix.Store.Translator.Term do
  ...
  defmacro __using__(_) do
    quote location: :keep do
      ...
      @spec serialize_key(Mnemonix.Store.t(), Mnemonix.key()) :: term | no_return
      def serialize_key(store, key) do
        :erlang.term_to_binary(key, serialization_opts(store))
      end

      @impl Mnemonix.Store.Translator
      @spec serialize_value(Mnemonix.Store.t(), Mnemonix.value()) :: term | no_return
      def serialize_value(store, value) do
        :erlang.term_to_binary(value, serialization_opts(store))
      end

      @impl Mnemonix.Store.Translator
      @spec deserialize_key(Mnemonix.Store.t(), term) :: Mnemonix.key() | no_return
      ...
      @impl Mnemonix.Store.Translator
      @spec deserialize_value(Mnemonix.Store.t(), term) :: Mnemonix.value() | no_return
